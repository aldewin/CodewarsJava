...
public class KataDirectionsReduction {
    ...
    public static String[] dirReduc(String[] arr) {
        ...
        List<String> shortWay = new ArrayList<>();
        for (String partWorld : arr) {
            if (shortWay.size() != 0 && directions.get(partWorld).equals(shortWay.get(shortWay.size() - 1)))
                shortWay.remove(shortWay.size() - 1);
            else
                shortWay.add(partWorld);
        }

        String[] result = new String[shortWay.size()];
        for (int i = 0; i < shortWay.size(); i++)
            result[i] = shortWay.get(i);
        return result;
    }
}
