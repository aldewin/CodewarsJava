...
public class KataBuddyPairs {
    public static void main(String[] args) {
        System.out.println(buddy(1131918, 1137924));
    }

    public static String buddy(long start, long limit) {
        for (long i = start; i <= limit; i++) {
            long sumDividers1 = sumDividers(i);
            long number = sumDividers1 - 1;

            if (sumDividers1 - 1 > i && sumDividers(number) == i + 1)
                return String.format("(%d %d)", i, number);
        }
        return "Nothing";
    }

    private static long sumDividers(long number) {
        long sum = 1;
