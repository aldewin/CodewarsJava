...
public class KataSumDigNth {
    public static void main(String[] args) {
        System.out.println(sumDigNthTerm(10, new long[] {2, 1, 3}, 6));
    }

    public static int sumDigNthTerm(long initval, long[] patternl, int nthterm) {
        int counter = 0;

        for (int i = 1; i < nthterm ; i++) {
            initval += patternl[counter++];
            if (counter == patternl.length) counter = 0;
        }

        String stringInitval = String.valueOf(initval);
        int sum = 0;
        for (int i = 0; i < stringInitval.length(); i++) {
            sum += Integer.parseInt(String.valueOf(stringInitval.charAt(i)));
        }
        return sum;
    }
}
